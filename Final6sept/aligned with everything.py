...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 01:00:15 2023
...
class PersonalityTypeApp:
    ...
    def display_predictions(self, num_predictions=3):
        ...
        openai_frame.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")
        ...
        ml_text.grid(row=0, column=0, padx=(0, 10), sticky="w")
        ml_predictions = ""
        ...
        for i in range(num_predictions):
            ...
            if image_path:
                ...
                image_label.grid(row=i + 1, column=0, padx=4, pady=0)

        # Extract keywords from the input text
        extracted_keywords = self.extract_keywords(preprocessed_text)

        # Create a label to display the extracted keywords
        keywords_frame = ttk.Frame(ml_frame)
        keywords_frame.grid(row=num_predictions - 3, column=3, padx=0, pady=0, sticky="w")
        
        keywords_label = ttk.Label(keywords_frame, text="Extracted Keywords that influenced the prediction result", font=("Arial", 12))
        keywords_label.grid(row=0, column=0, padx=0, pady=0, sticky="w")
        
        keywords_text = tk.Text(keywords_frame, height=5, width=82, wrap=tk.WORD)
        keywords_text.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        keywords_text.insert(tk.END, "\n".join(extracted_keywords))
        keywords_text.config(state=tk.DISABLED)  # Disable text editing

        # Create a list of labels and sizes for the treemap
        ...
        # Create a subplot for the treemap
        ...
        # Create a FigureCanvasTkAg
        treemap_canvas = FigureCanvasTkAgg(plt.gcf(), master=ml_frame)
        treemap_canvas.get_tk_widget().grid(row=1, column=3, padx=15, pady=0)
