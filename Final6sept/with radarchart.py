...
class PersonalityTypeApp:
    ...
    def create_radar_chart(self, window, categories, scores):
        ...
        # Ensure that scores have the same length as categories
        while len(scores) < N:
            scores.append(0.0)
        ...
        radar_chart_image = Image.open(buf)
        radar_chart_image = ImageTk.PhotoImage(radar_chart_image)

        radar_label = ttk.Label(window, image=radar_chart_image)
        radar_label.image = radar_chart_image
        radar_label.pack()
    ...
    def display_predictions(self, num_predictions=3):
        ...
        personality_images = {
            'ENFJ': 'enfj.png', 'ENFP': 'enfp.png', 'ENTJ': 'entj.png', 'ENTP': 'entp.png',
            'ESFJ': 'esfj.png', 'ESFP': 'esfp.png', 'ESTJ': 'estj.png', 'ESTP': 'estp.png',
            'INFJ': 'infj.png', 'INFP': 'infp.png', 'INTJ': 'intj.png', 'INTP': 'intp.png',
            'ISFJ': 'isfj.png', 'ISFP': 'isfp.png', 'ISTJ': 'istj.png', 'ISTP': 'istp.png',
        }
        ...
        # Radar chart for visualizing confidence scores
        radar_window = tk.Toplevel(result_window)
        radar_window.title("Radar Chart")

        # Define categories and scores for the radar chart
        radar_categories = [label_mapping[p] for p in top_n_predictions]
        radar_scores = top_n_probs.tolist()

        # Create the radar chart and display it
        self.create_radar_chart(radar_window, radar_categories, radar_scores)
