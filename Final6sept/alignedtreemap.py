...
class PersonalityTypeApp:
    ...
    def display_predictions(self, num_predictions=3):
        ...
        openai_text.grid(row=0, column=0, padx=(0, 10), sticky="w")
        ...
        ml_predictions_label.grid(row=1, column=0, sticky="w")
        ...
        for i in range(num_predictions):
            ...
            if image_path:
                ...
                image_label.grid(row=i + 1, column=0, padx=4, pady=10)
        ...
        squarify.plot(sizes=treemap_sizes, label=treemap_labels, alpha=0.7, color=['red', 'green', 'blue'],text_kwargs={'fontdict': {'weight': 'bold'}})
        plt.axis('off')

        # Set the title for the treemap
        plt.title("ML Predictions")

        # Create a FigureCanvasTkAgg to display the treemap in Tkinter
